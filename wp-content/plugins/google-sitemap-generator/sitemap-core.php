...
/** Sitemap Upload Dir */ 
define('SITEMAP_DIR','saestor://'.SAE_STORAGE);
/** Sitemap URL PATH */ 
define('SITEMAP_URL', 'http://' . $_SERVER['HTTP_APPNAME'] . '-'.SAE_STORAGE.'.stor.sinaapp.com/');
...
class GoogleSitemapGenerator {
	...
	*/
	function GetXmlUrl() {
		$name = SITEMAP_URL. $this->GetOption("b_filename");
	...
	*/
	function GetXmlPath() {
		$name = SITEMAP_DIR . '/' . $this->GetOption("b_filename");
		if(substr($name,-4)!=".xml") {
			$name.=".xml";
		}
		return $name;
	...
	*/
	...
	function IsFileWritable($filename) {
		if ( substr($filename, 0, 10) == 'saestor://' ) {
			return true;
		}else{
			return false;
		}
	...
	function BuildSitemap() {
		...
		$tmpfile = tempnam(SAE_TMP_PATH, 'SITEMAP_GZ');
		if($this->IsGzipEnabled()) {
			$fileName = $this->GetZipPath();
			$status->StartZip($this->GetZipPath(),$this->GetZipUrl());
			if($this->IsFileWritable($fileName)) {
				$this->_fileZipHandle = gzopen($tmpfile,"w1");
